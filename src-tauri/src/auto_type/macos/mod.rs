...
pub fn active_window_titles() -> kp_service::Result<Vec<WindowInfo>> {
    ...
    let excludes = vec![
        Some("OneKeePass".into()),
        Some("OneKeePass.app".into()),
        Some("onekeepass-frontend".into()),
    ];
    let mut infos = vec![];
    for w in widows.as_slice() {
        // log::debug!( "Window name: {:?}, owner: {:?}, pid: {}", w.owner.as_ref().map(|s| s.to_string()),w.name.as_ref().map(|s| s.to_string()), w.pid);
    ...
    // infos.iter().for_each(|info|  log::debug!("BEFORE Windows Info {:?}", &info));
    ...
    // infos.iter().for_each(|info|  log::debug!("AFTER Windows Info {:?}", &info));
