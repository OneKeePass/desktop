cfg_if::cfg_if! {
    if #[cfg(all(target_os = "macos", feature = "mas-build"))] {
        ...
        pub(crate) use bookmarks::BookmarkHandle;
            ...
            pub(crate) fn store_success_handles(
                ...
                if let Some(handle) = self.db_file.take() {
                    app_state.store_scoped_access(
                        ScopedAccessKey::Db(db_file_name.to_string()),
                        handle,
                    );
                }
        ...
        #[allow(dead_code)]
        pub(crate) mod bookmarks {
            pub(crate) fn create(_path: &str) -> Option<String> { None }
            pub(crate) fn store_backup_dir(_path: &str, _b64: &str) {}
            pub(crate) fn load_backup_dir(_path: &str) -> Option<String> { None }
            ...
            pub(crate) fn load_browser_dir(_browser_id: &str) -> Option<String> { None }
