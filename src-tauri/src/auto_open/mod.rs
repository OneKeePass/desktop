...
use onekeepass_core::db_service::{
    self as kp_service, AllTags, EntryCategories, EntryCategoryGrouping, EntryTypeHeaders,
    GroupTree, KdbxLoaded,
};
...
pub(crate) fn open_all_auto_open_dbs(
    ...
    let _root_dir_access = mas::auto_open_root_dir_access(auto_open_db_key);
    ...
    for entry_uuid in entry_uuids {
        ...
        if let Some(db_key_to_open) = ao_resolved.url_field_value {
            ...
            if !kp_service::is_db_opened(&db_key_to_open) {
                let mut scoped_access = mas::prepare_auto_open_db(&db_key_to_open, kf_path);
                ...
                match kp_service::load_kdbx(&db_key_to_open, password, kf_path) {
                    ...
                    Ok(kdbx_loaded) => match load_init_data(&grouping_kind, &kdbx_loaded) {
                        Ok(auto_open_db) => {
                            mas::store_auto_open_success_handles(
                                &mut scoped_access,
                                &db_key_to_open,
                                kf_path,
                                app_state,
