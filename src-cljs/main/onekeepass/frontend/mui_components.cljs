...
;; DateAdapter is the DateFnsUtils class (CJS module.exports = DateFnsUtils, so :as gives the class directly)
(def date-adapter ^js/DateAdapter DateAdapter)
...
;; SplitPane is imported via :default
(def split-pane
...
;; AutoSizer is imported via :default
(def auto-sizer
  "A reagent component formed from react component AutoSizer"
  (reagent.core/adapt-react-class AutoSizer))
