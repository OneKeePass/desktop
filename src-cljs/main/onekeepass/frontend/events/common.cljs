   ...
   {:db (assoc-in-key-db db [:locked] false)
    :fx [;; TODO: Combine these reset calls with 'common/kdbx-database-loading-complete'
         ...
         (if (get-in db [(active-db-key db) :external-change-pending])
           [:dispatch [:common/check-external-change-pending (active-db-key db)]]
...
(reg-event-fx
 :common/db-file-changed-externally
 (fn [{:keys [db]} [_event-id db-key]]
   (cond
     (and (= db-key (active-db-key db)) (not (locked? db)))
     {:fx [[:dispatch [:common/show-external-db-change-dialog db-key]]]}
...
(reg-event-fx
 :common/check-external-change-pending
 (fn [{:keys [db]} [_event-id db-key]]
...
(reg-event-fx
 :common/show-external-db-change-dialog
 (fn [{:keys [db]} [_event-id db-key]]
...
;; User chose "Merge"
(reg-event-fx
 :common/external-change-merge-start
 (fn [{:keys [_db]} [_event-id db-key]]
   ...
   (bg-merging/merge-kdbx-with-disk-version
    ...
    (fn [api-response]
      ...
      (when-some [merge-result (check-error api-response
                                            #(dispatch [:common/external-change-merge-error %]))]
        (dispatch [:common/external-change-merge-completed merge-result]))))))

(reg-event-fx
 :common/external-change-merge-completed
...
(reg-event-fx
 :common/external-change-merge-error
...
;; User chose "Reload"
(reg-event-fx
 :common/external-change-reload-start
 (fn [{:keys [db]} [_event-id db-key]]
...
(defn external-change-merge-start [db-key]
  (dispatch [:common/external-change-merge-start db-key]))

(defn external-change-reload-start [db-key]
  (dispatch [:common/external-change-reload-start db-key]))

(defn external-change-ignore [db-key]
  (dispatch [:common/external-change-ignore db-key]))
 ...
 (fn [{:keys [db]} [_event-id db-key]]
   {:db (assoc db :current-db-file-name db-key)
    :fx [(when (get-in db [db-key :external-change-pending])
           [:dispatch [:common/check-external-change-pending db-key]])]}))
